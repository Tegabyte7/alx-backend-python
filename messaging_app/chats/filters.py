...
class MessageFilter(django_filters.FilterSet):
    ...
    def filter_time_range(self, queryset, name, value):
        ...
        if value == 'today':
            today = timezone.now().date()
            return queryset.filter(sent_at_date=today)
        elif value == 'week':
            week_ago = today - timedelta(day=7)
    ...
    def filter_search(self, queryset, name, value):
        return queryset.filter(message_body_icontains=value)
...
class ConversationFilter(django_filters.FilterSet):
    ...
    class Meta:
        model = Conversation
        field = ['participant', 'date', 'date_range']

    def filter_by_participant(self, queryset, name, value):
        return queryset.filter(
            Q(participants_email_icontains=value) |
            Q(participants_username_icontains=value)
        )